...
const Valoration = ({ documentId, onCommentSuccess }) => {
  ...
  const [user, setUser] = useState({
    avatarUrl: 'https://via.placeholder.com/150', // Avatar del Usuario que esta en teoria conectado
    userId: "66ebbc56e9670a5556f9781a", //UserID que en teoria esta conectado
  });

  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await instance.get(`/Usuarios/id/${user.userId}`);
        setUser((prev) => ({
          ...prev,
          avatarUrl: response.data.perfil.fotoPerfil || prev.avatarUrl,
        }));
      } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (rating === 0) {
      setErrorMessage("Por favor, selecciona una valoración");
      return false;
    }
